...
void set_pixel (int i, int j, byte value);
    ...
    /**
      ...
      * @post Al ser una función void modifica la imagen objeto
      */
    void Invert();

    // Modifica el contraste de una Imagen .
    /**
     * @brief Lleva a cabo diversas operaciones para mdificar el contraste de una imagen en función de los valores introducidos.
     * @param in1 byte que representa el mínimo valor de la entrada
     * @param in2 byte que representa el máximo valor de la entrada
     * @param out1 byte que representa el valor mínimo de la salida
     * @param out2 byte que representa el valor máximo de la salida
     * @pre 0<=in1 && in1 <=255 && 0<=in2 && in2 <=255 && 0<=out1 && out1 <=255 && 0<=out2 && out2 <=255 && in1<in2 && out1<out2
     * @return devuelve un objeto imagen con el contraste cambiado en función de los parámetros.
     */
    ...
    /**
     * @brief Calcula la media de los píxeles de una imagen, según los parámetros.
     * @param i posición de la fila de donde comenzar
     * @param j posición de la columna de donde empezar.
     * @param height altura de la imagen.
     * @param width ancho de la imagen.
     * @return devuelve un valor double, resultado de la media aritmética.
     */
    double Mean (int i, int j, int height, int width) const;

    // Genera un icono como reducción de una imagen.
    /**
     * @brief Genera un icono resultado de la reducción de una imagen.
     * @param factor valor entorno al cual se realiza la reducción.
    ...
    /**
     * @brief Genera una subimagen a partir del objeto imagen.
     * @param nrow posición de la fila en la matriz en donde comenzar.
     * @param ncol posición de la columna de la matriz en donde comenzar.
     * @param height altura de la subimagen.
     * @param width ancho de la subimagen.
     * @pre   nrow>=0 && ncol>=0 && height>=0 && width>=0
     * @return devuelve la subimagen creada a partir de la imagen objeto en función de los parámetros.
     */
    Image Crop(int nrow, int ncol, int height, int width) const;

    // Genera una imagen aumentada 2x.
    /**
     * @brief Genera una imagen a la cual se le ha aplicado un zoom de 2x.
     * @return devuelve la imagen aumentada en un 2x. 
     */
    Image Zoom2X() const;
     ...
     * @return Devuelve una imagen resultado de barajar pseudoaleatoriamente las filas de esta.
     */
    void ShuffleRows();
} ;
